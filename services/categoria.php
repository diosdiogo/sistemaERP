<?php
    ...
    if($method == 'GET'){
        ...
        $categorias =$pdo->prepare("SELECT id,descricao FROM produtocategoria where idmatriz = :matriz and deletado = 'N' order by descricao asc;");
    ...
    if($method == 'POST'){
        ...
        $sql = "INSERT INTO produtocategoria (descricao, ativo, idmatriz, idempresa, created_at, deletado) 
        VALUES (?, 1, ?, ? , ?, 'N');
        ";
        ...
        if($row >= 1){
           ...
           $msg = 'Categoria salva nome ' . $dados['categ'] . '';
