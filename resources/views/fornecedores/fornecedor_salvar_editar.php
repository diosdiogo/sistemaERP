    ...
    if ($del == null) {
        ...
        */
        ...
        if ($cod == 0) {
            ...
            cidade, endereco, numero, complemento, cep, limitecredito, created_at, updated_at, idcidade, observacao, fornecedor) 
            VALUES (1, ?, ?, ?, ?, ?, ?, '1', '1', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,'S');";
            ...
            if ($row > 0) {
                $_SESSION['titulo'] = 'OK';
                $_SESSION['MsgRetorno'] = 'Fornecedor '.$nomeRF .', salvo com sucesso';
                $_SESSION['tipo'] = 'success';
                echo "<script>location.href='".$url."/fornecedores?r=true'</script>";
            }else{
                $_SESSION['titulo'] = 'ERRO';
                $_SESSION['MsgRetorno'] = 'Fornecedor '.$nomeRF .', não pode ser salvo';
                $_SESSION['tipo'] = 'error';
                echo "<script>location.href='".$url."/fornecedores?r=true'</script>";
            }
            ...
            if ($row >= 0) {
                $_SESSION['titulo'] = 'OK';
                $_SESSION['MsgRetorno'] = 'Fornecedor '.$nomeRF .', alterado com sucesso';
                $_SESSION['tipo'] = 'success';
                echo "<script>location.href='".$url."/fornecedores?r=true'</script>";
            }else{
                $_SESSION['titulo'] = 'ERRO';
                $_SESSION['MsgRetorno'] = 'Fornecedor '.$nomeRF .', não pode ser alterado';
                $_SESSION['tipo'] = 'error';
                echo "<script>location.href='".$url."/fornecedores?r=true'</script>";
            }
        }
        ...
        if ($row >= 0) {
            $_SESSION['titulo'] = 'OK';
            $_SESSION['MsgRetorno'] = 'Fornecedor '.$nomeRF .', deletado com sucesso';
            $_SESSION['tipo'] = 'success';
            echo "<script>location.href='".$url."/fornecedores?r=true'</script>";
        }else{
            $_SESSION['titulo'] = 'ERRO';
            $_SESSION['MsgRetorno'] = 'Fornecedor '.$nomeRF .', não pode ser deletado';
            $_SESSION['tipo'] = 'error';
            echo "<script>location.href='".$url."/fornecedores?r=true'</script>";
        }
