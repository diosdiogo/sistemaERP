<?php
    ...
    if ($del == null) {
        ...
        $limiteCredito = str_replace($pont,"", $_POST['limiteCredito']);
        $obs = $_POST['obs'];
        ...
        if ($cod == 0) {
            ...
            cidade, endereco, numero, complemento, cep, limitecredito, created_at, updated_at, idcidade, observacao, cliente) 
            VALUES (1, ?, ?, ?, ?, ?, ?, '1', '1', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,'N');";
            ...
            if ($row > 0) {
                $_SESSION['titulo'] = 'OK';
                $_SESSION['MsgRetorno'] = 'Cliente '.$nomeRF .', salvo com sucesso';
                $_SESSION['tipo'] = 'success';
                echo "<script>location.href='".$url."/clientes?r=true'</script>";
            }else{
                $_SESSION['titulo'] = 'ERRO';
                $_SESSION['MsgRetorno'] = 'Cliente '.$nomeRF .', não pode ser salvo';
                $_SESSION['tipo'] = 'error';
                echo "<script>location.href='".$url."/clientes?r=true'</script>";
            }
            ...
            if ($row >= 0) {
                $_SESSION['titulo'] = 'OK';
                $_SESSION['MsgRetorno'] = 'Cliente '.$nomeRF .', alterado com sucesso';
                $_SESSION['tipo'] = 'success';
                echo "<script>location.href='".$url."/clientes?r=true'</script>";
            }else{
                $_SESSION['titulo'] = 'ERRO';
                $_SESSION['MsgRetorno'] = 'Cliente '.$nomeRF .', não pode ser alterado';
                $_SESSION['tipo'] = 'error';
                echo "<script>location.href='".$url."/clientes?r=true'</script>";
            }
        }
        ...
        if ($row >= 0) {
            $_SESSION['titulo'] = 'OK';
            $_SESSION['MsgRetorno'] = 'Cliente '.$nomeRF .', deletado com sucesso';
            $_SESSION['tipo'] = 'success';
            echo "<script>location.href='".$url."/clientes?r=true'</script>";
        }else{
            $_SESSION['titulo'] = 'ERRO';
            $_SESSION['MsgRetorno'] = 'Cliente '.$nomeRF .', não pode ser deletado';
            $_SESSION['tipo'] = 'error';
            echo "<script>location.href='".$url."/clientes?r=true'</script>";
        }
